...
export function useBlockchainTransactions() {
  ...
  const estimatePostFee = useCallback(
    async (
      message: string,
      options: Partial<CreatePostTransactionParams> = {},
    ): Promise<number> => {
  ...
  const checkUserExists = useCallback(
    async (userWallet?: string): Promise<boolean> => {
  ...
  const upvote = useCallback(
    async (targetWallet: string): Promise<TransactionResponse> => {
  ...
  const createTip = useCallback(
    async (
      params: CreateTipTransactionParams,
    ): Promise<TransactionResponse> => {
  ...
  const updateVerification = useCallback(
    async (
      params: UpdateVerificationTransactionParams,
    ): Promise<TransactionResponse> => {
      return executeTransaction(
...
export function useCreatePost() {
  const {createPost, estimatePostFee, loading, error, connected, resetError} =
  ...
  const createPostWithFee = useCallback(
    async (message: string) => {
      resetError();
...
export function useVoting() {
  ...
  const voteWithPostCheck = useCallback(
    async (targetWallet: string, voteType: 'upvote' | 'downvote') => {
      resetError();
      ...
      if (postCount === 0) {
        throw new Error(
          'You must create at least one post before you can vote on other users',
        );
      }
  ...
  const upvoteWithCheck = useCallback(
  ...
  const downvoteWithCheck = useCallback(
    async (targetWallet: string) => {
 ...
 * Supports both SOL and SPL token tips
 */
export function useTipping() {
  const {createTip, loading, error, connected, resetError} =
    useBlockchainTransactions();
  ...
  const sendTip = useCallback(
    async (
      ...
      amount: number, // Amount in base units (lamports for SOL, smallest unit for SPL tokens)
      tokenMint?: string, // Optional: SPL token mint address (undefined for SOL)
      tokenSymbol?: string, // Optional: Token symbol for logging
      message?: string, // Optional: Message to include with tip
      targetType?: string, // Optional: Type of target (user, post, etc.)
      ...
      // For now, we only support SOL tips through the blockchain service
      // SPL token support can be added later when the API supports it
      if (tokenMint) {
        throw new Error(
          `SPL token tips are not yet supported. Please use SOL for now.`,
        );
      }
